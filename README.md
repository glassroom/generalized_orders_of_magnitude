...
For applications that require more precision, we provide an alternate broadcastable implementation of LMME via composition of vmapped vector operations: `goom.alternate_log_matmul_exp`. It is _much slower_ than the current default implementation, especially for larger input tensors. In practice, we have found its use unnecessary. Please see its docstring for usage details.
